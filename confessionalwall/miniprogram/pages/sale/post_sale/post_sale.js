  ...
  post: function () {
    ...
    db.collection('saleFriend').add({
      ...
      success(res) {
        ...
        var _id = res._id
        console.log(_id, attachments)
        attachments.forEach((value,index) => {
          const cloudPath = "saleFriend/" + year + "/" + month + "/" + day + "/" + getUUID() + "." + getExt(value.url);
          console.log(value)
          wx.cloud.uploadFile({
            // 指定上传到的云路径
            cloudPath: cloudPath,
            ...
            success: res => {
              ...
              if (res.errMsg == "cloud.uploadFile:ok") {
                ...
                db.collection('saleFriend').doc(_id).update({
                  // data 传入需要局部更新的数据
                  data: {
                    "attachments": attachments,
                  },
